...
class testRealEmail_traffic(unittest.TestCase):
    def setUp(self):
        ...
        """
        self.conn = msc.Connect(**login_info)
        self.curs = self.conn.cursor()
        self.curs.execute("DROP TABLE IF EXISTS message")
        self.conn.commit()
        curs.execute(TBLDEF)
        conn.commit()
        files = glob(FILESPEC)
        ...
        for f in files:
            ...
            id = self.msgids[msg['message-id']] = maildb.store(msg, self.conn, self.curs) # <---- CHANGED
    ...
    def test_message_ids(self):
        ...
        for message_id in self.msgids.keys():
            id, msg = maildb.msg_by_id(self.msgids[message_id], self.conn, self.curs) # <---- CHANGED
    ...
    def test_ids(self):
        ...
        for id in self.message_ids.keys():
            id1, msg = maildb.msg_by_message_id(self.message_ids[id], self.conn, self.curs) # <---- CHANGED
    ...
    def test_dates(self):
        ...
        self.assertEqual(self.rowcount,
                         len(maildb.msgs(self.conn, self.curs, mindate=mindate,
                                         maxdate=maxdate))) # <---- CHANGED
    def tearDown(self):
        self.conn.close()
