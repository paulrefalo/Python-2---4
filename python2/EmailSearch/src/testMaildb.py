...
class testRealEmail_traffic(unittest.TestCase):
    def setUp(self):
        ...
        for f in files:
            ...
            id = self.msgids[msg['message-id']] = maildb.store(msg, self.conn, self.curs)
    ...
    def test_message_ids(self):
        ...
        for message_id in self.msgids.keys():
            id, msg = maildb.msg_by_id(self.msgids[message_id], self.conn, self.curs) 
    ...
    def test_ids(self):
        ...
        for id in self.message_ids.keys():
            id1, msg = maildb.msg_by_message_id(self.message_ids[id], self.conn, self.curs)
